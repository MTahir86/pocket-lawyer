...
const TITLE = "Pocket Lawyer AI — Understand any document in simple language";
const DESCRIPTION =
...
function Index() {
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      const res = await analyze({ data: { files: payload, language } });
      setResult(res);
      setStatus("done");
    } catch (e) {
        ...
        {status === "done" && result && (
          <>
            <div className="flex items-center justify-between gap-3 rounded-2xl border border-border bg-card px-4 py-3">
              ...
              >
                <RotateCcw className="size-3.5" /> {t.newDoc}
              </button>
            </div>
